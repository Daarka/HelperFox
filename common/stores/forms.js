...
class Form extends DataObject {
	...
	async getEmbed() {
		...
		return {
			type: 17,
			accent_color: parseInt(!this.open ? 'aa0000' : this.color || '770000', 16),
			components: comps
		}
	}
	...
	async getInfo() {
		...
		return {
			type: 17,
			accent_color: parseInt(!this.open ? 'aa5555' : this.color || '770000', 16),
			components: comps
		}
	}
						...
						await msg.edit({
							components: [
								emb,
								{
									...
									components: [{
										type: 2,
										label: form.button_text ?? 'Register!',
										emoji,
