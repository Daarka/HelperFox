...
class ResponseHandler {
	...
	async startResponse(ctx) {
		...
		try {
			...
			if(cfg?.embed || form.embed) {
				...
				var fembeds = fcomps.map( c => {
					...
					return {
						components: [{
							type: 17,
							accent_color: parseInt(form.color || 'aa0000', 16),
							components: [
								...cmps//,
								//...c
							]
			...
			if (e.message === 'Cannot send messages to this user') {
				return 'Please turn on Direct Messages from Server Members in your Privacy Settings so that the bot can message you!';
			}
	...
	async sendQuestion(response, message) {
		...
		if(question) {
			...
			var template = {
				...
				color: parseInt(response.form.color || 'aa0000', 16)
			};
	...
	async buildResponseEmbeds(response, template) {
		...
		var comps = await this.bot.utils.genComps(questions, (q, i) => {
			...
			return {
				type: 17,
				accent_color: template?.color ?? parseInt(response.form.color || 'dd0000', 16),
				components: [
					...(template?.components ?? []),
					...c,
					...(template?.footer ?? [])
	...
	async sendResponse(response, message, user, config) {
		...
		try {
			...
			var template = {
				...
				color: parseInt('dd0000', 16),
				footer: [{
					type: 10,
					content:
						`PENDING • Form: ${response.form.hid} • Response: ${created.hid} `
				}]
			}
			var embeds = await this.buildResponseEmbeds(response, template);
			await prompt.edit({
			...
			if(!guild) return Promise.reject("ERR! Server not found! Aborting!");
			var chan_id = response.form.channel_id || config?.response_channel;
			var channel = guild.channels.resolve(chan_id);
			if(!channel) return Promise.reject("ERR! Server response channel missing! Aborting!");
			var toSend = {
				flags: ['IsComponentsV2'],
				components: [
					embeds[0],
		...
		return {
			msg: 'Response sent! Thanks!' +
				 '\nYou\'ll get another notification here when Daarka is able to log your submission. ❤',
			success: true
		}
	}
			...
			await prompt.edit({
				components: [{
					...
					components: [{
						...
						content:
							`## Response cancelled\n` +
							`This form response has been cancelled!\n` +
			...
			await prompt.edit({
				components: [{
					...
					components: [{
						...
						content:
							`## Response cancelled\n` +
							`Left the server; this form response has been automatically cancelled!\n` +
							`-# <t:${Math.floor(new Date().getTime() / 1000)}:F>`
					}]
				}]
			});
